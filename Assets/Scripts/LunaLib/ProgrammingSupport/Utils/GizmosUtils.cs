...
namespace LunaLib {
    public static class GizmosUtils {
        ...
        public static void DrawLocalRectXY(Transform space, Rect rect) => DrawRect(space.TransformPoint(rect.center), space.TransformVector(rect.size), space.rotation);
        ...
        public static void DrawArc(Vector3 center, float radius, Quaternion rotation, Vector3 startDir, Vector3 axis, float arcAngle, float maxAmgleStep) {
            ...
            List<Vector3> path = new (Enumerable.Range(0, segmentCount + 1).Select(i => center + (Quaternion.AngleAxis(arcAngle * i / segmentCount, axis) * baseVector)));
            DrawPath(path, false);
        }
