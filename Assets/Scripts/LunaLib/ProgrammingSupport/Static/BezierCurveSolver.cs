...
namespace LunaLib {
    public static class BezierCurveSolver {
        ...
        public static List<T> DiscretizeQuadraticBezier<T>(T p1, T p2, T p3, int subdivisions, Func<T, T, float, T> lerpFunc) {
            return new(Enumerable.Range(0, subdivisions + 2).Select(i => InterpolateQuadraticBezier(p1, p2, p3, (float)i / (subdivisions + 1), lerpFunc)));
        }
        public static List<T> DiscretizeCubicBezier<T>(T p1, T p2, T p3, T p4, int subdivisions, Func<T, T, float, T> lerpFunc) {
            return new(Enumerable.Range(0, subdivisions + 2).Select(i => InterpolateCubicBezier(p1, p2, p3, p4, (float)i / (subdivisions + 1), lerpFunc)));
        }
        public static List<T> DiscretizeBezier<T>(List<T> p, int subdivisions, Func<T, T, float, T> lerpFunc) {
            return new(Enumerable.Range(0, subdivisions + 2).Select(i => InterpolateBezier((float)i / (subdivisions + 1), lerpFunc, p)));
        }
        ...
        public static List<float> DiscretizeQuadraticBezier(float p1, float p2, float p3, int subdivisions) {
            return new(Enumerable.Range(0, subdivisions + 2).Select(i => InterpolateQuadraticBezier(p1, p2, p3, (float)i / (subdivisions + 1))));
        }
        public static List<float> DiscretizeCubicBezier(float p1, float p2, float p3, float p4, int subdivisions) {
            return new(Enumerable.Range(0, subdivisions + 2).Select(i => InterpolateCubicBezier(p1, p2, p3, p4, (float)i / (subdivisions + 1))));
        }
        public static List<float> DiscretizeBezier(List<float> p, int subdivisions) {
            return new(Enumerable.Range(0, subdivisions + 2).Select(i => InterpolateBezier((float)i / (subdivisions + 1), p)));
        }
        ...
        public static Vector3 InterpolateQuadraticBezierTangent(Vector3 p1, Vector3 p2, Vector3 p3, float lerpValue) {
            Vector3 a1 = Vector3.Lerp(p1, p2, lerpValue);
            Vector3 a2 = Vector3.Lerp(p2, p3, lerpValue);
            return (a2-a1).normalized;
        }
        public static Vector3 InterpolateCubicBezierTangent(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, float lerpValue) {
            Vector3 a1 = Vector3.Lerp(p1, p2, lerpValue);
            Vector3 a2 = Vector3.Lerp(p2, p3, lerpValue);
            Vector3 a3 = Vector3.Lerp(p3, p4, lerpValue);
            return InterpolateQuadraticBezierTangent(a1, a2, a3, lerpValue);
        }
        public static Vector3 InterpolateBezierTangent(float lerpValue, List<Vector3> p) {
            if(p.Count == 2) {
                return (p[1] - p[0]).normalized;
            }
            return InterpolateBezierTangent(lerpValue, p.MergedInPairs((v1, v2) => Vector3.Lerp(v1, v2, lerpValue)));
        }
